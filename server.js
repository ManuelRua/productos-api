...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      'GET /pagoQR - Imagen QR de pago (archivo directo)',
      'GET /pagoQR/bytes - Imagen QR de pago en bytes (base64)'
    ]
  });
});
...
// NUEVO: Endpoint para obtener imagen QR de pago como archivo (para WhatsApp)
  ...
  db.get('SELECT img FROM pago WHERE nombre = ?', ['pagoQR'], (err, row) => {
    if (err) {
      ...
      res.set({
        ...
        'Content-Disposition': 'inline; filename="pagoQR.jpg"'
      });
...
// NUEVO: Endpoint para obtener imagen QR de pago en bytes (para desarrollo)
app.get('/pagoQR/bytes', (req, res) => {
  db.get('SELECT img FROM pago WHERE nombre = ?', ['pagoQR'], (err, row) => {
    if (err) {
      res.status(500).json({ error: err.message });
    } else if (!row) {
